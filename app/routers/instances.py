...
def _instance_to_response(instance: MinerInstance) -> InstanceResponse:
    """Build response with streamers from config.json. Uses DB flag for status."""
    streamers = []
    try:
...
async def start_instance(
    ...
    return InstanceStatus(id=instance_id, is_running=True, pid=pid)
...
async def stop_instance(
    ...
    if not stopped:
        raise HTTPException(409, "Instance was not running")

    return InstanceStatus(id=instance_id, is_running=False, pid=None)
...
async def stream_logs(
    ...
    async def event_generator():
        async for line in tail_log(instance_id):
